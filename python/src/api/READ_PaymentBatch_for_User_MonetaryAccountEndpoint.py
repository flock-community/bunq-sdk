...
class READ_PaymentBatch_for_User_MonetaryAccountEndpoint (Wirespec.Endpoint):
  ...
  class Request(Wirespec.Request[None]):
    ...
    def __init__(self, userID: int, monetaryaccountID: int, itemId: int, CacheControl: Optional[str], UserAgent: str, XBunqLanguage: Optional[str], XBunqRegion: Optional[str], XBunqClientRequestId: Optional[str], XBunqGeolocation: Optional[str], XBunqClientAuthentication: str):
      self.path = READ_PaymentBatch_for_User_MonetaryAccountEndpoint.Request.Path(userID = userID, monetaryaccountID = monetaryaccountID, itemId = itemId)
      self.queries = READ_PaymentBatch_for_User_MonetaryAccountEndpoint.Request.Queries()
      self.headers = READ_PaymentBatch_for_User_MonetaryAccountEndpoint.Request.Headers(  CacheControl = CacheControl,
        UserAgent = UserAgent,
  ...
  def to_raw_request(serialization: Wirespec.Serializer, request: Request) -> Wirespec.RawRequest:
    return Wirespec.RawRequest(
      path = ["user", str(request.path.userID), "monetary-account", str(request.path.monetary-accountID), "payment-batch", str(request.path.itemId)],
  ...
  class Response200(Wirespec.Response[PaymentBatchRead]):
    ...
    def __init__(self, XBunqClientResponseId: Optional[str], XBunqClientRequestId: Optional[str], XBunqServerSignature: Optional[str], body: PaymentBatchRead):
      self.headers = READ_PaymentBatch_for_User_MonetaryAccountEndpoint.Response200.Headers(  XBunqClientResponseId = XBunqClientResponseId,
        XBunqClientRequestId = XBunqClientRequestId,
  ...
  class Response400(Wirespec.Response[READ_PaymentBatch_for_User_MonetaryAccount400ResponseBody]):
    ...
    body: READ_PaymentBatch_for_User_MonetaryAccount400ResponseBody = None
    status: int = 400
    headers: Headers = None
    ...
    def __init__(self, XBunqClientResponseId: Optional[str], XBunqClientRequestId: Optional[str], XBunqServerSignature: Optional[str], body: READ_PaymentBatch_for_User_MonetaryAccount400ResponseBody):
      self.headers = READ_PaymentBatch_for_User_MonetaryAccountEndpoint.Response400.Headers(  XBunqClientResponseId = XBunqClientResponseId,
        XBunqClientRequestId = XBunqClientRequestId,
        XBunqServerSignature = XBunqServerSignature)
      self.body = body
  ...
  @abstractmethod
  def READ_PaymentBatch_for_User_MonetaryAccount(self, req: Request) -> Response: pass
