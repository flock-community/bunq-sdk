...
class UPDATE_Company_for_UserEndpoint (Wirespec.Endpoint):
  ...
  class Request(Wirespec.Request[Company]):
    ...
    def __init__(self, userID: int, itemId: int, CacheControl: Optional[str], UserAgent: str, XBunqLanguage: Optional[str], XBunqRegion: Optional[str], XBunqClientRequestId: Optional[str], XBunqGeolocation: Optional[str], XBunqClientAuthentication: str, body: Company):
      self.path = UPDATE_Company_for_UserEndpoint.Request.Path(userID = userID, itemId = itemId)
      self.queries = UPDATE_Company_for_UserEndpoint.Request.Queries()
      self.headers = UPDATE_Company_for_UserEndpoint.Request.Headers(  CacheControl = CacheControl,
        UserAgent = UserAgent,
  ...
  class Response200(Wirespec.Response[CompanyUpdate]):
    ...
    def __init__(self, XBunqClientResponseId: Optional[str], XBunqClientRequestId: Optional[str], XBunqServerSignature: Optional[str], body: CompanyUpdate):
      self.headers = UPDATE_Company_for_UserEndpoint.Response200.Headers(  XBunqClientResponseId = XBunqClientResponseId,
        XBunqClientRequestId = XBunqClientRequestId,
  ...
  class Response400(Wirespec.Response[UPDATE_Company_for_User400ResponseBody]):
    ...
    body: UPDATE_Company_for_User400ResponseBody = None
    status: int = 400
    headers: Headers = None
    ...
    def __init__(self, XBunqClientResponseId: Optional[str], XBunqClientRequestId: Optional[str], XBunqServerSignature: Optional[str], body: UPDATE_Company_for_User400ResponseBody):
      self.headers = UPDATE_Company_for_UserEndpoint.Response400.Headers(  XBunqClientResponseId = XBunqClientResponseId,
        XBunqClientRequestId = XBunqClientRequestId,
        XBunqServerSignature = XBunqServerSignature)
      self.body = body
  ...
  @abstractmethod
  def UPDATE_Company_for_User(self, req: Request) -> Response: pass
