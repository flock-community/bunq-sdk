...
class READ_PaymentServiceProviderCredentialEndpoint (Wirespec.Endpoint):
  ...
  class Request(Wirespec.Request[None]):
    ...
    def __init__(self, itemId: int, CacheControl: Optional[str], UserAgent: str, XBunqLanguage: Optional[str], XBunqRegion: Optional[str], XBunqClientRequestId: Optional[str], XBunqGeolocation: Optional[str], XBunqClientAuthentication: str):
      self.path = READ_PaymentServiceProviderCredentialEndpoint.Request.Path(itemId = itemId)
      self.queries = READ_PaymentServiceProviderCredentialEndpoint.Request.Queries()
      self.headers = READ_PaymentServiceProviderCredentialEndpoint.Request.Headers(  CacheControl = CacheControl,
        UserAgent = UserAgent,
  ...
  def to_raw_request(serialization: Wirespec.Serializer, request: Request) -> Wirespec.RawRequest:
    return Wirespec.RawRequest(
      path = ["payment-service-provider-credential", request.path.itemId],
  ...
  class Response200(Wirespec.Response[PaymentServiceProviderCredentialRead]):
    ...
    body: PaymentServiceProviderCredentialRead = None
    status: int = 200
    headers: Headers = None
    ...
    def __init__(self, XBunqClientResponseId: Optional[str], XBunqClientRequestId: Optional[str], XBunqServerSignature: Optional[str], body: PaymentServiceProviderCredentialRead):
      self.headers = READ_PaymentServiceProviderCredentialEndpoint.Response200.Headers(  XBunqClientResponseId = XBunqClientResponseId,
        XBunqClientRequestId = XBunqClientRequestId,
  ...
  class Response400(Wirespec.Response[READ_PaymentServiceProviderCredential400ResponseBody]):
    ...
    body: READ_PaymentServiceProviderCredential400ResponseBody = None
    status: int = 400
    headers: Headers = None
    ...
    def __init__(self, XBunqClientResponseId: Optional[str], XBunqClientRequestId: Optional[str], XBunqServerSignature: Optional[str], body: READ_PaymentServiceProviderCredential400ResponseBody):
      self.headers = READ_PaymentServiceProviderCredentialEndpoint.Response400.Headers(  XBunqClientResponseId = XBunqClientResponseId,
        XBunqClientRequestId = XBunqClientRequestId,
        XBunqServerSignature = XBunqServerSignature)
      self.body = body
  ...
  @abstractmethod
  def READ_PaymentServiceProviderCredential(self, req: Request) -> Response: pass
