...
from ..model.DeviceServerRead import DeviceServerRead
from ..model.READ_DeviceServer400ResponseBody import READ_DeviceServer400ResponseBody

class READ_DeviceServer (Wirespec.Endpoint):
  @dataclass
  class Request(Wirespec.Request[None]):
    ...
    def __init__(self, itemId: int, CacheControl: Optional[str], UserAgent: str, XBunqLanguage: Optional[str], XBunqRegion: Optional[str], XBunqClientRequestId: Optional[str], XBunqGeolocation: Optional[str], XBunqClientAuthentication: str):
      self._path = READ_DeviceServer.Request.Path(itemId = itemId)
      self._queries =READ_DeviceServer.Request.Queries()
      self._headers = READ_DeviceServer.Request.Headers(  CacheControl = CacheControl,
        UserAgent = UserAgent,
  ...
  class Response200(Wirespec.Response[DeviceServerRead]):
    ...
    def __init__(self, XBunqClientResponseId: Optional[str], XBunqClientRequestId: Optional[str], XBunqServerSignature: Optional[str], body: DeviceServerRead):
      self._headers = READ_DeviceServer.Response200.Headers(  XBunqClientResponseId = XBunqClientResponseId,
        XBunqClientRequestId = XBunqClientRequestId,
  ...
  class Response400(Wirespec.Response[READ_DeviceServer400ResponseBody]):
    ...
    def __init__(self, XBunqClientResponseId: Optional[str], XBunqClientRequestId: Optional[str], XBunqServerSignature: Optional[str], body: READ_DeviceServer400ResponseBody):
      self._headers = READ_DeviceServer.Response400.Headers(  XBunqClientResponseId = XBunqClientResponseId,
        XBunqClientRequestId = XBunqClientRequestId,
        XBunqServerSignature = XBunqServerSignature)
      self._body = body
  ...
  class Handler(Wirespec.Endpoint.Handler):
    @abstractmethod
    def READ_DeviceServer(self, req: 'READ_DeviceServer.Request') -> 'READ_DeviceServer.Response': pass

  class Convert(Wirespec.Endpoint.Convert[Request, Response]):
    @staticmethod
    def to_raw_request(serialization: Wirespec.Serializer, request: 'READ_DeviceServer.Request') -> Wirespec.RawRequest:
    ...
    @staticmethod
    def from_raw_request(serialization: Wirespec.Deserializer, request: Wirespec.RawRequest) -> 'READ_DeviceServer.Request':
      return READ_DeviceServer.Request(
    ...
    @staticmethod
    def to_raw_response(serialization: Wirespec.Serializer, response: 'READ_DeviceServer.Response') -> Wirespec.RawResponse:
      match response:
        case READ_DeviceServer.Response200():
        ...
        case READ_DeviceServer.Response400():
          return Wirespec.RawResponse(
            status_code = response.status,
            headers = {"X-Bunq-Client-Response-Id": serialization.serialize_param(response.headers.XBunqClientResponseId, str), "X-Bunq-Client-Request-Id": serialization.serialize_param(response.headers.XBunqClientRequestId, str), "X-Bunq-Server-Signature": serialization.serialize_param(response.headers.XBunqServerSignature, str)},
    ...
    @staticmethod
    def from_raw_response(serialization: Wirespec.Deserializer, response: Wirespec.RawResponse) -> 'READ_DeviceServer.Response':
      match response.status_code:
        case 200:
          return READ_DeviceServer.Response200(
        ...
        case 400:
          return READ_DeviceServer.Response400(
