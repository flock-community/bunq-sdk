package com.bunq.sdk;

import org.junit.jupiter.api.Test;
import static org.junit.jupiter.api.Assertions.*;

public class BunqServerTest {

    private static final String PRIVATE_KEY_PEM = """
            -----BEGIN PRIVATE KEY-----
            
            -----END PRIVATE KEY-----
            """;
            
    private static final String PUBLIC_KEY_PEM = """
            -----BEGIN PUBLIC KEY-----
            
            rQIDAQAB
            -----END PUBLIC KEY-----
            """;

    @Test
    public void testBunqServerConstants() {
        assertNotNull(BunqServer.BUNQ_PUBLIC_SERVER);
        assertNotNull(BunqServer.BUNQ_SANDBOX_SERVER);
        
        assertEquals("https://api.bunq.com/v1/", BunqServer.BUNQ_PUBLIC_SERVER.baseUrl());
        assertEquals("https://public-api.sandbox.bunq.com/v1/", BunqServer.BUNQ_SANDBOX_SERVER.baseUrl());
        
        assertNotNull(BunqServer.BUNQ_PUBLIC_SERVER.pinnedKey());
        assertNotNull(BunqServer.BUNQ_SANDBOX_SERVER.pinnedKey());
    }

    @Test
    public void testConfigDefaultConstructorUsesSandbox() {
        Config config = new Config(
                BunqServer.BUNQ_SANDBOX_SERVER,
                "TestService",
                "test-api-key",
                PRIVATE_KEY_PEM,
                PUBLIC_KEY_PEM
        );

        assertEquals(BunqServer.BUNQ_SANDBOX_SERVER, config.bunqServer());
        assertEquals("TestService", config.serviceName());
        assertEquals("test-api-key", config.apiKey());
    }

    @Test
    public void testConfigExplicitBunqServerConstructor() {
        Config config = new Config(
                BunqServer.BUNQ_PUBLIC_SERVER,
                "TestService",
                "test-api-key",
                PRIVATE_KEY_PEM,
                PUBLIC_KEY_PEM
        );

        assertEquals(BunqServer.BUNQ_PUBLIC_SERVER, config.bunqServer());
        assertEquals("TestService", config.serviceName());
        assertEquals("test-api-key", config.apiKey());
    }

    @Test
    public void testGetPublicKeyAsString() {
        Config config = new Config(
                BunqServer.BUNQ_SANDBOX_SERVER,
                "TestService",
                "test-api-key",
                PRIVATE_KEY_PEM,
                PUBLIC_KEY_PEM
        );

        String publicKeyString = config.signingKeys().publicKeyPem();
        assertEquals(PUBLIC_KEY_PEM, publicKeyString);
        assertTrue(publicKeyString.contains("-----BEGIN PUBLIC KEY-----"));
        assertTrue(publicKeyString.contains("-----END PUBLIC KEY-----"));
    }
}